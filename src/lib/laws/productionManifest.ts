...
export function lawsForVeoJson(): Record<string, unknown> {
  return {
    ...
    // one-location / relative-placement / door-operation / carried-objects /
    // background-hold / chained-continuity / people-never-teleport.
    // NOTE: keep this slice in sync when adding a law — a stale end index
    // silently drops the newest guards from the exported JSON.
    action: PRODUCTION_LAWS.action_continuity_laws.laws.slice(0, 19),
