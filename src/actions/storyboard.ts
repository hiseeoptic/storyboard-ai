...
export async function generateStoryboardPlan(
  ...
  try {
    ...
    // The SCRIPT (text) is written by input.script_provider (default Claude
    // Opus 4.8); the storyboard IMAGES are always generated by the main
    // provider (Gemini). If the script model fails (e.g. ANTHROPIC_API_KEY not
    // set), fall back to the image provider so the flow still works.
    const scriptProvider = input.script_provider ?? provider;
    let breakdown;
    try {
      breakdown = await generateStoryboardBreakdown(enhanced, scriptProvider);
    } catch (e) {
      if (scriptProvider !== provider) {
        warnings.push(
          `Không viết được kịch bản bằng ${scriptProvider} — đã tự chuyển sang ${provider}. (${e instanceof Error ? e.message : String(e)})`
        );
